...
import {
  getAllConsumptionsBetweenDateInterval,
  updateConsumption,
} from "./ESConsumptionService";
...
export async function writeElectricityPriceForecastIntoDB(
  ...
  return new Promise((resolve, reject) => {
    ...
    const filenameExpression = new RegExp(
      energySystem.electricityPriceForecastFilename + ".*.csv?$"
    );
      ...
      try {
        ...
        parse(
          ...
          async (error, forecasts: priceForecast[]) => {
            ...
            // offset zwischen Verbrauchsdaten und zugeordneten Strompreisprognosewerten als Vielfaches von 24
            const offsetDays = Math.ceil(
              energySystem.optimierungshorizontMin / 60 / 24
            );
            ...
            for (const consumption of consumptions) {
              for (const forecast of forecasts) {
                const forecastDate = new Date(
                  consumption.date.getTime() + offset
                );
                if (
                  forecastDate.getHours() == forecast.date.getHours() &&
                  ...
                  await updateConsumption(
                    consumption,
                    consumption.energySystemId,
                    consumption.bufferIndex
                  );
                }
              }
            }

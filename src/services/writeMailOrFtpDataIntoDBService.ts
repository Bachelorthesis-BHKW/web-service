...
export async function writeMailOrFtpDataIntoDB(energySystem: EnergySystem): Promise<void> {
  return new Promise((resolve,reject)=>{
    ...
    try {
      const filePath = './tmp/' + file_date + '_' + energySystem.inputFilename + '.xlsx';
    ...
    if (energySystem.mailInputTrigger) {
      saveMailAttachments().then(function() {console.log('Mail attachments saved: Successful');
        onAttachmentsSaved()}).catch(function(){reject();console.log('Error saving mail attachments: Unsuccessful')});
    } else if (energySystem.ftpInputTrigger) {
      saveFtpAttachments(energySystem, file_date).then(function() {console.log('FTP data saved: Successful');
        onAttachmentsSaved()}).catch(function(){reject();console.log('Error saving ftp attachments: Unsuccessful')});
    } else {
    ...
    async function onAttachmentsSaved(){
      const xlsx_weather_data = './tmp/' + file_date + '_' + energySystem.inputFilename + '_weather_data' + '.xlsx';
      const xlsx_data = './tmp/' + file_date + '_' + energySystem.inputFilename + '.xlsx';
      ...
      try {
        ...
        try {
          const forecast_1 = XlsxToJsonTimestamp(xlsx_weather_data, 'Tabelle1', 'A11:B58');
          const forecast_2 = XlsxToJson(xlsx_weather_data, 'Tabelle1', 'C11:D58', ['temperature','globalHorizontalIrradiance']);
          const forecast = [];
          ...
          for (const hourlyForecast of forecast){
            await createWeatherForecast(
              { date: hourlyForecast.date, temperature: hourlyForecast.temperature, globalHorizontalIrradiance: hourlyForecast.globalHorizontalIrradiance},
        ...
        const consumptions_1 = XlsxToJsonTimestamp(xlsx_data, 'Tabelle1', cellrange_date);
        const consumptions_2 = XlsxToJson(xlsx_data, 'Tabelle1', cellrange_consumptions, ['verbrauchStrom', 'verbrauchHeizung', 'verbrauchBww', 'aussentemperatur']);
        const consumptions = [];
        ...
        // BHKW 1
        await writeComponentCurrentsFromMailToDB(xlsx_data, 'Tabelle1', cell_chp1_id, cellrange_date,
        ...
        // TES
        await writeComponentCurrentsFromMailToDB(xlsx_data, 'Tabelle1', cell_tes_id, cellrange_date,
          cellrange_tes, ["component_id"], ["speicherfuellstand"])

        // SLK
        try{
          await writeComponentCurrentsFromMailToDB(xlsx_data, 'component_currents_slk', 'A2',
            'A10:B110', 'C10:F110',
            ["component_id"],
            ["laufzeit"])
        } catch {
...
async function writeComponentCurrentsFromMailToDB(xlsx_data: string, sheet_currents: string, sheet_range_component_id: string,
                                                  sheet_range_component_currents_1: string, sheet_range_component_currents_2: string,
                                                  sheet_headers_component_id: string[],
                                                  sheet_headers_component_currents_2: string[]) {
  const component_id_json = XlsxToJson(xlsx_data, sheet_currents, sheet_range_component_id, sheet_headers_component_id);
  ...
  const component_currents_1 = XlsxToJsonTimestamp(xlsx_data, sheet_currents, sheet_range_component_currents_1);
  const component_currents_2 = XlsxToJson(xlsx_data, sheet_currents, sheet_range_component_currents_2, sheet_headers_component_currents_2);
  ...
  await addESComponentCurrentsToESComponent(component_id, component_currents).then(
    function (){console.log("Finished write "+ sheet_currents+ " to DB")}
  );
}
